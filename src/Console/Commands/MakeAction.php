    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (file_exists($template)) {
            ...
            if (!copy($template, $file_name)) {
                ...
                $output->writeln('Unable to create action: Failed to copy file');
            ...
            if (!file_put_contents($file_name, $contents)) {
                ...
                $output->writeLn('Unable to create action: Failed to write file.');

            }

            $output->writeln('Action created at: ' . strtolower(rtrim(get_config('app.namespace'), '\\')) . '/Actions/' . $name);

            return Command::SUCCESS;

        } else {

            $output->writeln('Unable to create action: Template not found');
