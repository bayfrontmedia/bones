    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (file_exists($template)) {
            ...
            if (!copy($template, $file_name)) {
                ...
                $output->writeln('<error>Unable to create exception: Failed to copy file</error>');
            ...
            if (!file_put_contents($file_name, $contents)) {
                ...
                $output->writeLn('<error>Unable to create exception: Failed to write file.</error>');

            }

            $output->writeln('<info>Exception created at: ' . strtolower(rtrim(get_config('app.namespace'), '\\')) . '/Exceptions/' . $name . '</info>');

            return Command::SUCCESS;

        } else {

            $output->writeln('<error>Unable to create exception: Template not found</error>');
