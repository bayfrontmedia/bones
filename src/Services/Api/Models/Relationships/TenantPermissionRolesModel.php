...
class TenantPermissionRolesModel extends ApiModel implements ScopedRelationshipInterface
{
    ...
    public function __construct(EventService $events, Db $db, Log $log, TenantPermissionsModel $tenantPermissionsModel, TenantRolesModel $tenantRolesModel)
    {
    ...
    public function add(string $scoped_id, string $resource_id, array $relationship_ids): void
    {
        ...
        if (in_array(Api::ACTION_UPDATE, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Permission added to tenant roles', [
                'action' => 'api.tenant.permission.roles.add',
                'tenant_id' => $scoped_id,
                'permission_id' => $resource_id,
    ...
    public function getCollection(string $scoped_id, string $resource_id, array $args = []): array
    {
        ...
        if (in_array(Api::ACTION_READ, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Tenant permission roles read', [
                'action' => 'api.tenant.permission.roles.read',
                'tenant_id' => $scoped_id,
                'permission_id' => $resource_id,
    ...
    public function remove(string $scoped_id, string $resource_id, array $relationship_ids): void
    {
        ...
        if (in_array(Api::ACTION_UPDATE, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Permission removed from tenant roles', [
                'action' => 'api.tenant.permission.roles.remove',
                'tenant_id' => $scoped_id,
                'permission_id' => $resource_id,
                'role_ids' => $relationship_ids
            ]);
