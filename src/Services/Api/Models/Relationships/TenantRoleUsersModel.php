...
class TenantRoleUsersModel extends ApiModel implements ScopedRelationshipInterface
{
    ...
    public function __construct(EventService $events, Db $db, Log $log, TenantRolesModel $tenantRolesModel, TenantUsersModel $tenantUsersModel)
    {
    ...
    public function add(string $scoped_id, string $resource_id, array $relationship_ids): void
    {
        ...
        if (in_array(Api::ACTION_UPDATE, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Users added to tenant role', [
                'action' => 'api.tenant.role.users.add',
                'tenant_id' => $scoped_id,
                'role_id' => $resource_id,
    ...
    public function getCollection(string $scoped_id, string $resource_id, array $args = []): array
    {
        ...
        if (in_array(Api::ACTION_READ, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Tenant role users read', [
                'action' => 'api.tenant.role.users.read',
                'tenant_id' => $scoped_id,
                'role_id' => $resource_id,
    ...
    public function remove(string $scoped_id, string $resource_id, array $relationship_ids): void
    {
        ...
        if (in_array(Api::ACTION_UPDATE, App::getConfig('api.log.audit.actions'))) {
            ...
            $this->auditLogChannel->info('Users removed from tenant role', [
                'action' => 'api.tenant.role.users.remove',
                'tenant_id' => $scoped_id,
                'role_id' => $resource_id,
                'user_ids' => $relationship_ids
            ]);
